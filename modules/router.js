...
const writeInfluxBandwidth = (influxClient, bandwidth) => {
    influxClient.write('bandwidth').field(bandwidth).queue();
};
...
const writeInfluxNAS = (influxClient, nas) => {
    influxClient.write('nas').field(nas).queue();
};
...
const writeInflux = (influxClient, bandwidth, devices, overview, nas) => {
    ...
    influxClient
        .syncWrite()
        .then(() =>
            console.debug(`${Date.now()} router: influx write point success`)
        )
        .catch((error) =>
            console.debug(`${Date.now()} router: write failed ${error}`)
        );
};
